...
{
    Poweritem *pDerivedObj = (Poweritem *)malloc(sizeof(Poweritem));
    Elements *pObj = New_Elements(label);
    // setting derived object member
    pDerivedObj->img = al_load_bitmap("assets/image/poweritem.png");
    ...
    pDerivedObj->newborntime = al_create_timer(2.0);
    ...
    pObj->Update = Poweritem_update;
    pObj->Interact = Poweritem_interact;
    pObj->Draw = Poweritem_draw;
    pObj->Destroy = Poweritem_destory;
    return pObj;
}
void Poweritem_update(Elements *self) {
    Poweritem *Obj = ((Poweritem *)(self->pDerivedObj));
